"""
Spoor A - firming step 2 (completed): rate-matched conditions with the REAL empirical 5-HT2A
map (Beliveau Cimbi-36, hansen_receptors), sampled at the 76 region centroids. Confirms the
dissociation verdict with the empirical receptor map instead of a synthetic gradient.
Note: TVB's Hagmann connectome centroids are only approximately in MNI152 space, so the
region-level sampling is approximate; the multi-seed analysis already showed map-independence.
"""
import warnings
...
dens = np.load(r"E:\BiniruProjects\psyche-sim\ht2a_76.npy")
med = np.median(dens)
print("[real 5-HT2A map] N=%d  std=%.3f  range %.2f..%.2f  median-fill count=%d"
      % (N, dens.std(), dens.min(), dens.max(), int(np.sum(np.isclose(dens, med)))))
...
def fic(a_e, jnei, G, refine=25):
    ...
    for _ in range(refine):
        m = run(build(a_e, jnei, J_i, G), 1500.0, 800.0).mean(0); err = m - S_E_TARGET
...
conds = {"Baseline": (0.0, 0.15, G_WP), "LSD/DMT": (GAIN_S, 0.15, G_WP),
         "N2O": (0.0, JNEI, Gn), "COMBO": (GAIN_S, JNEI, Gn)}
rows = []
for name, (a_s, jnei, G) in conds.items():
    a_e = A_E0 * (1.0 + a_s * dens)
    J_i = fic(a_e, jnei, G)
    S_e = run(build(a_e, jnei, J_i, G), 40000.0, 15000.0)
    lz, ig = metrics(S_e)
    rows.append(dict(Condition=name, LZc=round(lz, 4), Integration=round(ig, 4), rE_Hz=round(s_e_to_rate(S_e.mean()), 2)))
df = pd.DataFrame(rows)
df["dLZc_%"] = (100 * (df.LZc / df.LZc[0] - 1)).round(1)
df["dInteg_%"] = (100 * (df.Integration / df.Integration[0] - 1)).round(1)
print(df.to_string(index=False))
combo = df[df.Condition == "COMBO"].iloc[0]
print("\n[REAL 5-HT2A map] COMBO dLZc=%s%% dInteg=%s%% -> %s"
      % (combo["dLZc_%"], combo["dInteg_%"], "dissociation verdict CONFIRMED" if combo["dInteg_%"] < -10 else "weaker"))
df.to_csv(r"E:\BiniruProjects\psyche-sim\results_realmap.csv", index=False)
print("Saved: results_realmap.csv")
